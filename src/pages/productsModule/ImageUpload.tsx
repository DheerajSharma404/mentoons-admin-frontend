    ...
    try {
      ...
      for (const file of files) {
        ...
        if (data.success && data.data.fileDetails) {
          // Create new image data object with the URL
          const newImageData: ImageData = {
            imageUrl: data.data.fileDetails.url,
          };

          // Update the product images array based on multiple flag
          if (multiple) {
            const newProductImages = [...productImages, newImageData];
            setProductImages(newProductImages);
            onImageUpload(newProductImages);
          } else {
            // If single image, replace existing
            const newProductImages = [newImageData];
            setProductImages(newProductImages);
            onImageUpload(newProductImages);
          }
          ...
          errorToast("Failed to process uploaded image");
        }
      }
    } catch (error) {
